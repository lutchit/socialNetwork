	...
	var collectionGroups = mongoose.model('groups', groupSchema);
  	...
  	var Groups = {
        ...
        get: function get(id, callback) {
            collectionGroups.findOne({ _id: id }).populate('members').exec(function(err, group) {
                if (err) {
        ...
        getAllDetailed: function getAll(callback) {
            collectionGroups.find({}, function(err, groups) {
                if (err) {
            		return callback(null, {
                		status: 500,
        ...
        update: function update(id, newName, newDescription, user, callback) {
        ...
        join: function join(userId, groupId, user, callback) {
        ...
        addComment: function addComment(groupId, message, idAuthor, user, callback) {
             collectionGroups.findOne({ _id: groupId }, function(err, group) {
                if(err) {
                    return callback(null, {
                        status: 500,
                ...
                if(group) {
                    ...
                    return group.save(function(err, groupUpdated) {
                        ...
                        return callback(commentToAdd, null);
                    });
                }
                return callback(null, {
                    status: 404,
        ...
        isMemberOf: function isMemberOf(groupId, userId, callback) {
            collectionGroups.findOne({ _id: groupId }, function(err, group) {
                if (err) {
            		return callback(null, {
                		status: 500,
                ...
                if(group) {
                    var user = group.members.indexOf(userId);
                    if(user > -1) {
